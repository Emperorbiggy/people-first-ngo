...
export default function PaidPartyAgents({ history = [], stats = {} }) {
    ...
    return (
        <AdminLayout title="Paid Party Agents">
            <div className="max-w-6xl mx-auto space-y-6">

                <div>
                    <h1 className="text-xl font-bold text-gray-800">Paid Party Agents</h1>
                    <p className="text-sm text-gray-500 mt-0.5">All party agent payment attempts processed via Paystack.</p>
                </div>
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    {history.length === 0 ? (
                        <div className="py-16 text-center">
                            <p className="text-gray-400 text-sm">No party agent payments have been made yet.</p>
                        </div>
                    ) : filtered.length === 0 ? (
                            ...
                            <table className="min-w-full">
                                <thead>
                                    <tr className="bg-gray-50 text-left">
                                        {['Name', 'Amount', 'Bank Name', 'Bank Code', 'Account Number', 'Account Name', 'Status', 'Date'].map((h) => (
                                ...
                                <tbody className="divide-y divide-gray-50">
                                    {filtered.map((h) => (
                                        <tr key={h.id} className="hover:bg-indigo-50/30 transition-colors">
                                            ...
                                            <td className="px-5 py-3 text-sm text-gray-600 whitespace-nowrap">{h.bank_name}</td>
                                            <td className="px-5 py-3 text-sm text-gray-600 whitespace-nowrap">{h.bank_code}</td>
                                            <td className="px-5 py-3 text-sm text-gray-600 whitespace-nowrap">{h.account_number}</td>
