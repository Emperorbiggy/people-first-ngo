...
export default function DataboyAccreditationPayments({ history = [], stats = {} }) {
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    {history.length === 0 ? (
                        <div className="py-16 text-center">
                            <p className="text-gray-400 text-sm">No databoy accreditation payments have been made yet.</p>
                        </div>
                    ) : filtered.length === 0 ? (
                            ...
                            <table className="min-w-full">
                                <thead>
                                    <tr className="bg-gray-50 text-left">
                                        {['Databoy', 'Payment Date', 'Amount', 'Account Number', 'Account Name', 'Status', 'Processed'].map((h) => (
                                    ...
                                    {filtered.map((h) => (
                                        <tr key={h.id} className="hover:bg-indigo-50/30 transition-colors">
                                            ...
                                            <td className="px-5 py-3 text-sm text-gray-600 whitespace-nowrap">
                                                {new Date(h.payment_date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}
                                            </td>
