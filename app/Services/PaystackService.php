...
class PaystackService
{
    ...
    /**
     * Transfer state looked up by OUR reference rather than Paystack's code.
     *
     * The fallback for payments where the transfer code was never captured —
     * the reference is generated by us before the call, so it always exists.
     */
    public function verifyTransferByReference(string $reference)
    {
        try {
            $response = Http::withHeaders([
                'Authorization' => 'Bearer ' . $this->secretKey,
            ...
            $this->logResponse('verifyTransferByReference', $response);
            ...
            return [
                'status'  => false,
                'message' => $response->json()['message'] ?? 'Unable to verify transfer',
            ];
        } catch (\Exception $e) {
            Log::error('Paystack verifyTransferByReference exception: ' . $e->getMessage());
