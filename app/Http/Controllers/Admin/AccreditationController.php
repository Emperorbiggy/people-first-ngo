...
class AccreditationController extends Controller
{
    ...
    {
        $applications = DataboyApplication::with(['databoy:id,full_name', 'lga:id,name', 'ward:id,name', 'accreditedBy:id,name'])
            ->where('is_accredited', true)
            ->orderByDesc('accredited_at')
            ->get([
                'id', 'full_name', 'calling_phone_number', 'registered_by', 'lga_id', 'ward_id',
                'passport_photograph_path', 'accredited_at', 'accredited_by',
            ]);
